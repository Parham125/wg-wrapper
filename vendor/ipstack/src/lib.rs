...
pub struct IpStackConfig {
    ...
    /// Idle timeout for TCP connections, applied per stream.
...
impl IpStackConfig {
    ...
    ///
    /// A TCP stream with no traffic in either direction for this long is reset. This takes
    /// precedence over [`TcpConfig::timeout`].
    ///
    /// # Arguments
    ///
    /// * `timeout` - The idle timeout duration for TCP connections
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let mut config = IpStackConfig::default();
    /// config.tcp_timeout(Duration::from_secs(7200));
    /// ```
...
fn create_stream(
    ...
    match packet.transport_header() {
        TransportHeader::Tcp(h) => {
            let stream = IpStackTcpStream::new(src_addr, dst_addr, h.clone(), up_pkt_sender, cfg.mtu, msgr, cfg.tcp_config.clone(), cfg.tcp_timeout)?;
